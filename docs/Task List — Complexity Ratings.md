...
| Done | #    | Task                                                  | Difficulty | Week | Notes                                                        |
| ---- | ---- | ----------------------------------------------------- | ---------- | ---- | ------------------------------------------------------------ |
| [x]  | DB-1 | Install PostgreSQL locally + enable PostGIS extension | M          | 3    | Each team member needs this before any local testing works   |
| [x]  | DB-2 | Run the schema SQL (create all 4 tables)              | E          | 3    | Script is ready in `database/` — just needs to be run        |
| [x]  | DB-3 | Verify PostGIS geo queries work (e.g. ST_Distance)    | M          | 4    | Test that coordinates can be stored and queried by proximity |
...
| [x]  | DB-6 | Migration strategy (how schema changes are shared)    | M          | 3    | Versioned SQL scripts in `database/migrations/`              |
...
| Done | #     | Task                                              | Difficulty | Week | Notes                                                                    |
| ---- | ----- | ------------------------------------------------- | ---------- | ---- | ------------------------------------------------------------------------ |
| [x]  | FE-1  | Get Expo app running on simulator / phone         | E          | 3    | Scaffolded but not confirmed running for all team members                |
| [x]  | FE-2  | Agree on folder/file structure                    | E          | 3    | Should mirror the backend areas: auth, profile, locations, reviews, feed |
| [x]  | FE-3  | Sketch 3 core screens (Login, Profile, Feed)      | E          | 3    | Even on paper — needed before building                                   |
...
| [ ]  | INT-3 | Send sprint plan to mentors (Dominic & Victor)                    | E          | 3    | Action item from Meeting 2 (March 12) — share this doc          |
| [x]  | INT-4 | Everyone: get project cloned and running locally                  | E          | 3    | Backend needs DB credentials; frontend needs Node + Expo CLI    |
